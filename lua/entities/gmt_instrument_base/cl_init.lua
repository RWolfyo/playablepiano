...
function ENT:IsKeyDown( key, shift )
	...
	if input.IsKeyDown( key ) and input.IsShiftDown( ) == shift then return true; end
	...
	if keyData.Label then
		...
		if keyDown then
			...
			if keyData.AColor then
				color = keyData.AColor;
			end
		else
			if keyData.Color then
				color = keyData.Color;
			end
		end

		--  Override positions, if needed
		if keyData.TextX then
			offsetX = keyData.TextX;
		end

		if keyData.TextY then
			offsetY = keyData.TextY;
		end
...
function ENT:DrawHUD( )
	...
	if self.AdvancedMode then
		...
		draw.DrawText( "MIDI: " .. text, "InstrumentKeyLabel", mainX + 15, mainY + mainHeight + 30, self.DefaultTextInfoColor, TEXT_ALIGN_LEFT );
	end
end
...
function ENT:PrecacheMaterials( )
	...
	for name, keyMaterial in pairs( self.KeyMaterials ) do
		--  TODO: what the fuck, this table is randomly created
...
function ENT:CaptureAllKeys( capture )
	...
	if g_dummy and g_dummy:IsValid( ) then
		g_dummy:Remove( );
	end
end
...
--  Called when they press shift
function ENT:ShiftMod( )
end

--  Called when they press cntrl
